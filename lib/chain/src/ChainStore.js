...
class ChainStore
{
            ...
            return db_api.exec( "get_objects", [ ["2.1.0"] ] ).then( optional_objects => {
                ...
                for(let i = 0; i < optional_objects.length; i++) {
                    ...
                    if( optional_object ) {
                        ...
                        if( delta < 60  ) {
                            safe_api() && Apis.instance().db_api().exec( "set_subscribe_callback", [ this.onUpdate.bind(this), true ] )
                            .then(() => {
    ...
    */
    getAsset( id_or_symbol )
    {
        ...
        safe_api() && Apis.instance().db_api().exec( "lookup_asset_symbols", [ [id_or_symbol] ] )
        .then( asset_objects => {
    ...
    */
    getAccountRefsOfKey( key )
    {
        ...
        else
        {
            ...
            safe_api() && Apis.instance().db_api().exec( "get_key_references", [ [key] ] )
            .then( vec_account_id => {
    ...
    getBalanceObjects( address )
    {
        ...
        if( current === undefined )
        {
            ...
            */
            ...
            safe_api() && Apis.instance().db_api().exec( "get_balance_objects", [ [address] ] )
    ...
    */
    fetchObject( id, force = false )
    {
        ...
        if( result === undefined ) { // the fetch
            ...
            safe_api() && Apis.instance().db_api().exec( "get_objects", [ [id] ] ).then( optional_objects => {
                //if(DEBUG) console.log("... optional_objects",optional_objects ? optional_objects[0].id : null)
                for(let i = 0; i < optional_objects.length; i++) {
                    let optional_object = optional_objects[i];
                    if( optional_object )
    ...
    */
    fetchWitnessByAccount( account_id )
    {
        return new Promise( (resolve,reject ) => {
            safe_api() && Apis.instance().db_api().exec( "get_witness_by_account", [ account_id ] )
    ...
    */
    ...
    {
        return new Promise( (resolve,reject ) => {
            safe_api() && Apis.instance().db_api().exec( "get_committee_member_by_account", [ account_id ] )
    ...
    */
    fetchFullAccount( name_or_id )
    {
        ...
        if( !this.fetching_get_full_accounts.has(name_or_id) || (Date.now() - this.fetching_get_full_accounts.get(name_or_id)) > 5000  ) {
            ...
            safe_api() && Apis.instance().db_api().exec("get_full_accounts", [[name_or_id],true])
            .then( results => {
                if(results.length === 0 ) {
                    if( ChainValidation.is_object_id(name_or_id) ) {
    ...
    */
    ...
    getObjectsByVoteIds( vote_ids )
    {
        ...
        if( missing.length ) {
            ...
            safe_api() && Apis.instance().db_api().exec( "lookup_vote_ids", [ missing ] )
            .then( vote_obj_array => {
    ...
    queryForSubjectPublish(pretictionEndTime){
        safe_api() && Apis.instance().db_api().exec("get_global_properties").then(chainGlobalProperties => {
            let vote_duration_percent = chainGlobalProperties.parameter.subject_profile.vote_duration_percent;
            safe_api() && Apis.instance().db_api().exec("get_dynamic_global_properties").then(chainDynamicProperties => {
                var result = {};
